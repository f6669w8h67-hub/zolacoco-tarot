...
const ASTRO_STORAGE_KEY = "zolacoco-astro-dice-records-v1";
...
export default function AstroDicePage(){
  ...
  useEffect(()=>{try{setHistory(JSON.parse(localStorage.getItem(ASTRO_STORAGE_KEY)??"[]"))}catch{setHistory([])}},[]);
  ...
  function save(){if(!roll)return;setSaved("idle");try{const item:HistoryItem={id:Date.now(),question:question.trim(),category,planet_name:roll.planet.name,sign_name:roll.sign.name,house_name:roll.house.name,note,created_at:new Date().toISOString()};const next=[item,...history].slice(0,80);localStorage.setItem(ASTRO_STORAGE_KEY,JSON.stringify(next));setHistory(next);setSaved("saved")}catch{setSaved("error")}}
  ...
  return <main className="astro-dice-page">
    <nav className="ad-nav"><Link className="ad-brand" href="/">ZOLACOCO <span>TAROT</span></Link><div><Link href="/">最初選單</Link><Link href="/#draw">塔羅抽牌</Link><Link href="/pendulum">靈擺</Link><Link className="ad-login" href="/admin">我的存檔</Link></div></nav>
