...
function ChoicePage({ view, step, title, copy, choices, value, onSelect, onBack, onNext, children }: { view: View; step: number; title: string; copy: string; choices: Choice[]; value: Choice | null; onSelect: (choice: Choice) => void; onBack: () => void; onNext: () => void; children?: React.ReactNode }) {
  return <section className="astro-step" style={{ "--scene": `url('${scenes[view]}')` } as React.CSSProperties}>
...
export default function AstrologyExplorationPage() {
  ...
  return <main className={`astro-page astro-view-${view}`}>
    {view === "intro" && <section className="astro-intro" style={{ "--scene": `url('${scenes.intro}')` } as React.CSSProperties}>
      <div className="astro-intro-art" aria-hidden="true" />
      <div className="astro-intro-copy"><small>ZOLACOCO · INNER ASTROLOGY JOURNEY</small><h1>探索你的星盤</h1><p className="astro-subtitle">看見你的人生課題、內在需求，<br />以及此刻最值得理解的自己。</p><div className="astro-about"><h2>這個體驗可以帶給你什麼？</h2><p>透過四個簡單提問，整理你目前最在意的生活主題、反覆感受與內在需要，最後找到適合帶進個人星盤解析的重點。</p><ul><li>約 3 分鐘完成</li><li>不需要先懂占星</li><li>不需要提供出生資料</li></ul></div><aside><b>開始以前，請先記得</b><p>跟著當下最真實的感覺回答即可。你不需要想得很完整，也不需要迎合任何理想答案。</p></aside><button className="astro-primary" onClick={() => setView("theme")}>開始探索自己 <b>→</b></button><button className="astro-text-button" onClick={() => setView("records")}>查看過往探索紀錄</button></div>
    </section>}
    ...
    {view === "result" && result && theme && state && need && focus && <section className="astro-result" style={{ "--scene": `url('${scenes.result}')` } as React.CSSProperties}>
    ...
    {view === "service" && result && <section className="astro-service" style={{ "--scene": `url('${scenes.service}')` } as React.CSSProperties}>
